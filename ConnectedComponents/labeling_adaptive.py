...
if __name__ == "__main__":
    ...
    adaptive_img = cv2.adaptiveThreshold(bin_img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 101, 40)
    #kernel = np.ones((5,5),np.uint8)
    #opening = cv2.morphologyEx(bin_img, cv2.MORPH_OPEN, kernel)#オープニング
    #bin_img = opening
    print("ret"+str(ret))
    retval, labels, stats, centroids = cv2.connectedComponentsWithStats(adaptive_img)
    ...
    cv2.imwrite("adaptive_bin_img.png", adaptive_img)
